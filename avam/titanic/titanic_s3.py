...
def print_scores(title, model, X, y):
    ...
    print('Precision score for base line model is : {0:.2f}'.format(precision_score(y, model.predict(X))))
    print('Recall score for base line model is : {0:.2f}'.format(recall_score(y, model.predict(X))))   
    print('******************************************************\n\n')
...
def run():
    # Load Processed data
    train_df = pd.read_csv(path.join(data_dir,'processed/train.csv'),index_col="PassengerId")
    test_df = pd.read_csv(path.join(data_dir,'processed/test.csv'), index_col='PassengerId')
    ...
    print_scores('1. LogisticRegression - Training Dataset',model_log_reg,X_test, y_test)
